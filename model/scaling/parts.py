...
class CrossScaleEmbedding(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int,
        ...
        self.conv_tansposes = nn.ModuleList()
    ...
    def forward(self, x, next: List[nn.Module] = []):
        ...
        first = next[0] if len(next) else None
        rest = next[1:]
        features = torch.zeros(
            (tokens.shape[0], tokens.shape[1] * 2, tokens.shape[2], tokens.shape[3]))
        # cat together in interdigital fashion on the channel dimension
        features[:, ::2, :, :] = tokens
        if first:
            features[:, 1::2, :, :] = first(features, rest)
        else:
            # just for test. in a normal net, it should not go here
            features[:, 1::2, :, :] = tokens
            print("Warning: no next layer")
        offset = 0
        y = torch.zeros_like(x)
        for i, d in enumerate(self.dim_list):
            y += self.conv_tansposes[i](features[:,
                                        offset:offset + 2 * d, :, :])
